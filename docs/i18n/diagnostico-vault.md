...
**Campo de nota com decalque real:** o corpo (`narrativa.pt.md`) soma 3.333 palavras nas 13 notas, e o grep dos dezenove padrões do catálogo não achou nenhuma ocorrência nos parágrafos. Mas o heading de abertura do corpo (`### <cargo> | <empresa>`) está em **inglês** em 13 das 13 notas publicadas — confirmado por `grep -h "^### " content/experiences/pt/*.md`, que devolve 13 linhas, todas em inglês — só reproduz o mesmo texto de `papel`/`empresa` sem traduzir, ex.: `### Senior Full Stack Developer | MedEspecialista`, `### Software Architect | Digidados`, `### Java Instructor | SENAI`. `manchester` é uma **14ª pasta** dentro de `Brag/`, fora da allow-list `ENGAGEMENTS_CONHECIDOS` e por isso não publicada — não é uma das 13. Ela serve como prova de que o padrão certo é alcançável no vault: seu heading já está corretamente em PT (`### Desenvolvedor / Arquiteto | Projeto Manchester (pesquisa clínica)`), mas esse padrão nunca foi aplicado retroativamente às 13 notas publicadas. Esse heading é texto do **corpo** da nota (dentro de `narrativa.pt.md`), não um campo de frontmatter, e o script não tem como derivá-lo sem que a nota o forneça.
Veredito parcial: **corrigir nota a nota.** São 13 notas, cada uma com uma linha de heading a reescrever (e, possivelmente, menções ao cargo em inglês espalhadas no restante do corpo, que não foram varridas linha a linha nesta tarefa) — escopo pequeno por nota, mas incorrigível a partir do script.
...
3. **Heading de abertura do corpo em `experiences`** — nota a nota, 13 notas, uma linha cada; usar a nota `manchester` (14ª pasta do vault, fora da allow-list, não publicada) como referência de forma correta.
