...
- [x] **HR-01** — texto do lead. **Mock:** *I take over platforms that have eroded to the point where no one can safely change them, and turn them into a delivery operation that runs on one meeting a month.* **Site:** o mesmo texto **precedido de** *Fractional software engineer and architect.* — que é exatamente o que o kicker âmbar logo acima já diz. · `src/messages/en.json` › `Home.hero.subtitle`
