...
- [ ] **PO-10** — `MANTER` (decidido em 2026-09-03): a margem que colapsa entrega os mesmos 24px na prática. ~~espaçamento entre blocos~~. **Mock:** `gap: 24px` uniforme entre elementos. **Site:** `margin: 24px 0` em cada parágrafo, que **colapsa** entre irmãos e dá 24px — equivalente na maioria dos casos, mas não onde há `<hr>` ou `<blockquote>` no meio. · `src/components/content/MDXContent.module.css`
