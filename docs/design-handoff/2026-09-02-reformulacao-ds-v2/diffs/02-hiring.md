...
- [ ] **HI-16** — valor da segunda métrica. **Mock:** `~5`. **Site:** `~5×/month` — e a legenda logo abaixo já diz *client-reported issues a month*, então a unidade aparece duas vezes. · `src/features/hiring/Evidence.js`
- [ ] **HI-17** — legenda da primeira métrica. **Mock:** *repositories under my ownership, 3 in active development*. **Site:** *Repositories owned*. · `src/messages/en.json`
- [ ] **HI-18** — legenda da terceira métrica. **Mock:** *between production releases*. **Site:** *Release cadence*. · `src/messages/en.json`
- [ ] **HI-19** — legenda da quarta métrica. **Mock:** *automated tests (was 70)*. **Site:** *Automated tests* — some o "antes", que é o que dá tamanho ao número. · `src/messages/en.json`
...
- [ ] **HI-24** — texto do cartão `Async operation`. **Mock:** *Requirements, architecture decisions, and backlog live in the repository as a single source of truth…* **Site:** *Async operation. Requirements, architecture decisions, …* — o texto repete a própria tag roxa acima dele. Mesmo padrão de `WM-04`/`WM-05`. · `src/messages/en.json`
