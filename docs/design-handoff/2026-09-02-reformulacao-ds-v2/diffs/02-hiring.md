...
- [x] **HI-23** — segundo parágrafo. **Mock:** um segundo parágrafo separado, `16px/1.7` em `#B4BCCA`, sobre ter feito a migração nos dois sentidos (Muvz → decompor, MedEspecialista → consolidar). **Site:** não existe; o conteúdo é um bloco único de 18px com vários `<strong>` de números embutidos no meio da prosa. · `src/messages/en.json` / `content/`
